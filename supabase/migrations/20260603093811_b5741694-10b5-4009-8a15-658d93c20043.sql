-- Attach audit-actor triggers so changed_by/created_by always fall back to auth.uid()
DROP TRIGGER IF EXISTS set_stock_history_actor ON public.stock_history;
CREATE TRIGGER set_stock_history_actor
BEFORE INSERT ON public.stock_history
FOR EACH ROW EXECUTE FUNCTION public.set_audit_actor_from_auth();

DROP TRIGGER IF EXISTS set_raw_material_stock_history_actor ON public.raw_material_stock_history;
CREATE TRIGGER set_raw_material_stock_history_actor
BEFORE INSERT ON public.raw_material_stock_history
FOR EACH ROW EXECUTE FUNCTION public.set_audit_actor_from_auth();

DROP TRIGGER IF EXISTS set_work_orders_actor ON public.work_orders;
CREATE TRIGGER set_work_orders_actor
BEFORE INSERT ON public.work_orders
FOR EACH ROW EXECUTE FUNCTION public.set_audit_actor_from_auth();

-- Backfill historical NULLs from related records
UPDATE public.stock_history sh
SET changed_by = p.created_by
FROM public.products p
WHERE sh.changed_by IS NULL AND sh.product_id = p.id AND p.created_by IS NOT NULL;

UPDATE public.raw_material_stock_history rh
SET changed_by = rm.created_by
FROM public.raw_materials rm
WHERE rh.changed_by IS NULL AND rh.raw_material_id = rm.id AND rm.created_by IS NOT NULL;

-- Fix auto clock-out to use Africa/Lagos local time instead of server UTC (en-NG locale)
...
BEGIN
  ...
  FOR r IN
    ...
    WHERE a.organization_id = _org_id
      ...
      AND (a.attendance_date < v_local_today
           OR (a.attendance_date = v_local_today
               AND v_local_time >= s.auto_clockout_time))
  LOOP
    v_clock_in := r.clock_in_time;
    v_clock_out := ((r.attendance_date::text || ' ' || r.auto_clockout_time::text)::timestamp AT TIME ZONE 'Africa/Lagos');
    ...
    IF r.overtime_start_time IS NOT NULL THEN
      v_ot_start := ((r.attendance_date::text || ' ' || r.overtime_start_time::text)::timestamp AT TIME ZONE 'Africa/Lagos');
