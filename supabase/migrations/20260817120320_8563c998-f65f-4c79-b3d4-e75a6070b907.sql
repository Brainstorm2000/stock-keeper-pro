...
CREATE INDEX IF NOT EXISTS idx_sales_org_created ON public.sales(organization_id, created_at);
CREATE INDEX IF NOT EXISTS idx_purchase_items_product ON public.purchase_items(product_id);
CREATE INDEX IF NOT EXISTS idx_purchases_org_date ON public.purchases(organization_id, purchase_date);
CREATE INDEX IF NOT EXISTS idx_stock_history_product_created ON public.stock_history(product_id, created_at);
CREATE INDEX IF NOT EXISTS idx_sale_return_items_product ON public.sale_return_items(product_id);
...
CREATE OR REPLACE FUNCTION public.stock_sales_performance_rows(
  _org_id uuid,
  _from timestamptz,
  _to timestamptz,
  _branch_id uuid DEFAULT NULL,
  ...
  _search text DEFAULT NULL,
  _fast_threshold numeric DEFAULT 1,
  _sort text DEFAULT 'product_name',
...
AS $$
  ...
  ORDER BY
    CASE WHEN _dir = 'asc' THEN
      CASE _sort
        WHEN 'product_name' THEN lower(b.product_name)
        WHEN 'category' THEN b.category
        WHEN 'stock_status' THEN b.stock_status
        WHEN 'velocity' THEN b.velocity
        ELSE NULL END
    END ASC NULLS LAST,
    ...
    CASE WHEN _dir = 'asc' THEN
      CASE _sort
        WHEN 'current_stock' THEN b.current_stock
        WHEN 'units_sold' THEN b.units_sold
        WHEN 'revenue' THEN b.revenue
        WHEN 'cogs' THEN b.cogs
        WHEN 'gross_profit' THEN b.gross_profit
        WHEN 'margin' THEN b.margin
        WHEN 'inventory_value' THEN b.inventory_value
        WHEN 'opening_stock' THEN b.opening_stock
        WHEN 'received' THEN b.received
        WHEN 'cost_price' THEN b.cost_price
        WHEN 'selling_price' THEN b.selling_price
        ELSE NULL END
    END ASC NULLS LAST,
    CASE WHEN _dir = 'desc' THEN
      CASE _sort
