...
export function BOMTab() {
  ...
  const addItem = () =>
    setItems([...items, { raw_material_id: "", quantity_required: 0 }]);
  const removeItem = (idx: number) =>
    setItems(items.filter((_, i) => i !== idx));
  const updateItem = (
    idx: number,
    field: keyof BOMItemForm,
    value: string | number,
  ) => {
    const updated = [...items];
    if (
      ...
      value &&
      updated.some((it, i) => i !== idx && it.raw_material_id === value)
    ) {
      toast({
    ...
    updated[idx] = {
      ...updated[idx],
      [field]: field === "quantity_required" ? Number(value) : value,
    };
    setItems(updated);
  };
  ...
  const handleSubmit = async () => {
    ...
    const validItems = items.filter(
      (i) => i.raw_material_id && i.quantity_required > 0,
    );
    ...
    if (new Set(ids).size !== ids.length) {
      toast({
        ...
        description: "Each material can only appear once.",
        variant: "destructive",
      });
      return;
    }
  ...
  return (
    <div className="space-y-4">
              ...
              filtered.map((b) => {
                ...
                return (
                  <TableRow key={b.id}>
                    ...
                    <TableCell className="text-right">
                      {formatCurrency(b.labor_cost_per_unit)}
                    </TableCell>
                    <TableCell className="text-right">
                      {formatCurrency(b.overhead_cost_per_unit)}
                    </TableCell>
      ...
      <Dialog
        open={!!detailBom}
        onOpenChange={(open) => !open && setDetailBom(null)}
      >
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>{detailBom?.name} — Materials</DialogTitle>
          </DialogHeader>
          <Table>
            <TableHeader>
              <TableRow>
            ...
            <TableBody>
              {detailBom?.bom_items?.map((item) => (
                <TableRow key={item.id}>
                  ...
                  <TableCell>
                    {item.raw_materials?.units?.abbreviation ||
                      item.raw_materials?.units?.name ||
                      "—"}
                  </TableCell>
                  <TableCell className="text-right">
                    {Number(item.quantity_required)}
                  ...
                  <TableCell className="text-right">
                    {Number(item.raw_materials?.current_stock || 0)}
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
      ...
      >
        <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {editing ? "Edit" : "Create"} Bill of Materials
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
                ...
                <div key={idx} className="flex gap-2 items-end">
                  <div className="flex-1">
                    <Select
                      ...
                      onValueChange={(v) =>
                        updateItem(idx, "raw_material_id", v)
                      }
                    >
                      <SelectTrigger>
                      ...
                      <SelectContent>
                        {materials
                          .filter(
                            (m) =>
                              m.id === item.raw_material_id ||
                              !items.some(
                                (it, i) =>
                                  i !== idx && it.raw_material_id === m.id,
                              ),
                          )
                          .map((m) => (
                  ...
                  <div className="w-24">
                    <Input
                      ...
                      onChange={(e) =>
                        updateItem(idx, "quantity_required", e.target.value)
                      }
                    />
                  </div>
                  <Button
          ...
          <DialogFooter>
            ...
            <Button
              ...
              className="bg-[#FF9E3D] hover:bg-[#e88d30] text-[#000B26] font-bold transition-colors"
              disabled={!name || !productId || items.length === 0}
            >
