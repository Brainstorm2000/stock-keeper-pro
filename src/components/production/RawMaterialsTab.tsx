...
export function RawMaterialsTab() {
  ...
  const handleStockUpdate = async () => {
    ...
    const newStock =
      stockChangeType === "increase" ? current + change : current - change;
  ...
  const totalValue = filtered.reduce(
    (sum, m) => sum + m.current_stock * m.cost_per_unit,
    0,
  );
  const lowStockCount = filtered.filter(
    (m) => getStockStatus(m) !== "normal",
  ).length;
    ...
    <div className="space-y-4">
              ...
              filtered.map((m) => {
                ...
                return (
                  <TableRow key={m.id}>
                    ...
                    <TableCell>
                      {m.units?.abbreviation || m.units?.name || "—"}
                    </TableCell>
      ...
      >
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            <DialogTitle>Adjust Stock — {stockMaterial?.name}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              Current stock:{" "}
              <span className="font-semibold text-foreground">
                {stockMaterial?.current_stock}
              </span>
            </p>
            <div className="space-y-2">
              <Label>Type</Label>
              <Select
                ...
                onValueChange={(v) =>
                  setStockChangeType(v as "increase" | "decrease")
                }
              >
                <SelectTrigger>
                  <SelectValue />
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={!stockChange || Number(stockChange) <= 0}
            >
              Update Stock
            </Button>
          </DialogFooter>
      ...
      >
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            <DialogTitle>Record Waste — {wasteMaterial?.name}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              Current stock:{" "}
              <span className="font-semibold text-foreground">
                {wasteMaterial?.current_stock}
              </span>
            </p>
            <div className="space-y-2">
              ...
              <Input
                type="number"
                min="1"
                max={wasteMaterial?.current_stock}
                value={wasteQty}
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setWasteDialogOpen(false)}>
              Cancel
            </Button>
            <Button
              variant="destructive"
              onClick={handleRecordWaste}
              disabled={!wasteQty || Number(wasteQty) <= 0}
            >
              Record Waste
            </Button>
          </DialogFooter>
