...
export function PurchaseDetailsDialog({ purchase, open, onOpenChange }: PurchaseDetailsDialogProps) {
  ...
  const hasPaymentChanges = paymentStatus !== purchase.payment_status || amountPaid !== String(purchase.amount_paid);
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            ...
            <div>
              ...
              <p className="font-medium">{format(new Date(purchase.purchase_date), 'MMMM d, yyyy')}</p>
            </div>
            <div>
              ...
              <TableBody>
                {purchase.purchase_items?.map(item => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell>
                      <Badge variant={item.products?.category === 'sellable' ? 'default' : 'secondary'}>
                        {item.products?.category || '—'}
                      </Badge>
                    </TableCell>
                    <TableCell className="text-right">
                      {Number(item.quantity).toLocaleString()} {item.products?.units?.abbreviation}
                    </TableCell>
                    <TableCell className="text-right">{formatCurrency(Number(item.unit_cost))}</TableCell>
                    <TableCell className="text-right font-medium">{formatCurrency(Number(item.total_cost))}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-between items-center">
              <div className="text-sm text-muted-foreground">
                Balance: {formatCurrency(Number(purchase.total_amount) - (Number(amountPaid) || 0))}
              </div>
              {hasPaymentChanges && (
