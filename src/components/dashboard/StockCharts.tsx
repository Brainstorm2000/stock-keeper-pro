...
export function StockCharts({ products }: StockChartsProps) {
  ...
  const barChartData = products.slice(0, 10).map((p) => ({
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      >
        ...
        <CardContent>
          {/* Ensure this div has a relative position and explicit height */}
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    ...
                    cy="50%"
                    innerRadius={60}
                    outerRadius={80} // Reduced slightly to prevent label clipping
                    paddingAngle={5}
                    dataKey="value"
                    label={({ name, percent }) =>
