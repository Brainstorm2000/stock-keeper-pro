...
export default function Expenses() {
  ...
  const filteredExpenses = useMemo(() => {
    return expenses.filter(expense => {
      ...
      const matchesBranch = filterBranch === 'all' || expense.branch_id === filterBranch;
  ...
  const handleSubmit = async () => {
    ...
    if (receiptFile) {
      try {
        setUploading(true);
        finalReceiptUrl = await uploadExpenseReceipt(receiptFile, organization.id);
      } catch (error) {
        ...
        <Card>
          <Table>
            ...
            <TableBody>
              {expensesLoading ? (
                <TableRow>
                  <TableCell colSpan={branches.length > 0 ? 7 : 6} className="text-center py-8">
                    <Loader2 className="h-6 w-6 animate-spin mx-auto text-muted-foreground" />
                  </TableCell>
                </TableRow>
              ) : filteredExpenses.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={branches.length > 0 ? 7 : 6} className="text-center py-8 text-muted-foreground">
                    No expenses found
                  </TableCell>
                </TableRow>
              ) : (
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              {receiptFile && (
                <p className="text-sm text-muted-foreground">New file: {receiptFile.name}</p>
              )}
            </div>
          </div>
            ...
            <div className="space-y-2">
              ...
              {categories.length === 0 ? (
                ...
                <div className="space-y-2 max-h-[200px] overflow-y-auto">
                  {categories.map((cat) => (
                    <div key={cat.id} className="flex items-center justify-between p-2 rounded bg-muted">
                      <span>{cat.name}</span>
                      <Button
                        variant="ghost"
                        size="sm"
