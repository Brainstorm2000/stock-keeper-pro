...
export default function POS() {
  ...
  const resolveTransactionBranch = ():
    ...
    if (ids.length > 1) {
      return {
        ok: false,
        message:
          "Cart contains items from multiple branches. Please clear the cart or select one branch and only sell items from that branch.",
      };
    }
    ...
    if (ids.length === 1 && branchId !== ids[0]) {
      return {
        ok: false,
        message:
          "This cart contains items from a different branch. Switch branch (or clear the cart) to continue.",
      };
    }
  ...
  const handleCheckout = async () => {
    ...
    if (isWalkIn && (paymentType === "partial" || paymentType === "credit")) {
      toast({
        title: "Walk-in customers cannot have partial or credit sales",
        description: "Please select a registered customer first.",
        variant: "destructive",
      });
      return;
    }
    ...
    if (!isWalkIn && computedBalance > 0) {
      ...
      if (selectedCustomer && Number(selectedCustomer.debt_limit) > 0) {
        ...
        const existingDebt = (outstandingSales || []).reduce(
          (sum, s) => sum + Number(s.balance_due),
          0,
        );
              ...
              {productsLoading ? (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                  {availableProducts.map((product) => {
                    const productType = (product as any).item_type || "product";
                    const sellingPrice =
                      Number((product as any).selling_price) || 0;
                              ...
                              <TooltipProvider>
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    <span className="font-medium text-sm line-clamp-2 cursor-help">
                                      {product.name}
                                    </span>
                                  </TooltipTrigger>
                                  <TooltipContent>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  className="data-[state=checked]:bg-[#FF9E3D] data-[state=checked]:border-[#FF9E3D] data-[state=unchecked]:border-[#FF9E3D] border-input transition-colors focus-visible:ring-[#FF9E3D]"
              ...
              <div className="space-y-2">
                ...
                {(() => {
                  ...
                  return (
                    <>
                      <div className="grid grid-cols-3 gap-2">
                        {(["full", "partial", "credit"] as const).map(
                          (type) => (
                            <Button
                              key={type}
                              type="button"
                              // Keep variant as default/outline for the base structure
                              variant={
                                paymentType === type ? "default" : "outline"
                              }
                      ...
                      {isWalkIn && (
                        <p className="text-xs text-muted-foreground">
                          Select a registered customer for partial or credit
                          sales
                        </p>
                      )}
                    </>
                ...
                <div className="space-y-2">
                  ...
                  {amountPaid > 0 && amountPaid < total && (
                    <p className="text-sm text-muted-foreground">
                      Balance due: {formatCurrency(total - amountPaid)}
                    </p>
                  )}
                </div>
              )}
              ...
              {hasStockWarning && (
                <div className="flex items-center gap-2 p-3 bg-destructive/10 border border-destructive/30 rounded-lg text-destructive text-sm">
                  ...
                  <span>
                    Some items exceed available stock. Proceeding may result in
                    negative inventory.
                  </span>
                </div>
              )}
